...
def analyze_text_with_llm(text):
    text_prompt = (
    f"This is a news article that talks about inflation, prices and markups. \n"
    ...
    f"inflation, prices, markups, etc."
    ...
    f"Inflation in Europe finally back on the ECB target, taking pressure off households and policymakers (Sentiment 8, Uncertainty 2). \n"
    f"Unexpected increases in food prices are feeding doubts among consumers (Sentiment 3, Uncertainty 7). \n"
    f"Technology firms report record markups thanks to efficiency gains (Sentiment 8, Uncertainty 2). \n"
    f"Energy crisis: production costs are shooting up in key sectors (Sentiment 2, Uncertainty 6). \n"
    f"Public policies are stabilizing the price of basic goods after months of high volatility, but serious doubts remain for the second half of the year (Sentiment 8, Uncertainty 8). \n"
    f"IMF alerts of possible economic stagnation due to persistent inflation (Sentiment 2, Uncertainty 7). \n"
    f"Moderate inflation fuels consumption and gives SMEs a very welcome break (Sentiment 8, Uncertainty 3). \n"
    f"Experts can't reach consensus on the whether the new monetary policies will be enough to contain inflation (Sentiment 5, Uncertainty 7). \n"
    f"Innovation in renewables promises moderate energy prices in the long run, but results will depend on a fast global deployment (Sentiment 7, Uncertainty 7). \n"
    f"Transmission of the oil price shock to consumer prices was moderate, which is good. But absortion by firm markups can't last long. (Sentiment 6, Uncertainty 6). \n"
